...
module inspection_unit#(
    ...
    output wire [DATA_WIDTH * 3 - 1:0] debug_data,
    output wire debug_search_ready,
    output wire [ENGINES_NUMBER-1:0] debug_engine_match,
...
reg [DATA_WIDTH-1:0] debug_buff[0:3];
reg [ADDR_WIDTH-1:0] index = 0;
assign debug_data = { debug_buff[0], debug_buff[1], debug_buff[2] };

always@(posedge axis_clk) begin
    if (s_axis_tvalid) begin
        debug_buff[index] <= s_axis_tdata;
        index <= index + 1;
        if (index >= 3) begin
            index <= 0;
        end
...
/////// some debug info for VIO
assign debug_search_ready = eng_search_ready;
assign debug_engine_match = engine_match;
assign debug_start_search = eng_start_search;
