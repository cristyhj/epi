...
{
	printk("epi: enters read: count = %d\n", count);
	...
	if (netfilter_log.current_read < netfilter_log.current_log) {
		retval = count < netfilter_log.lens[netfilter_log.current_read] ? 
					count : netfilter_log.lens[netfilter_log.current_read];
		
		if (copy_to_user(buf, netfilter_log.buff[netfilter_log.current_read], retval))
		{
			return 0;
		}
